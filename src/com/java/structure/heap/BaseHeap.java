 ...
 *  3. 节点的父节点是 （index-1）/2
 *  4.堆中的每一个节点的关键字都大于（或等于）这个节点的子节点的关键字。
 */
public class BaseHeap {
    ...
    /**
     * 向上调整(新增时需要向上调整，让当前插入的结点上移到最大的小于他的二节点之前)
     * 注意：向上筛选和向下不同，向上筛选只用和一个父节点进行比较，比父节点小就停止筛选了
     * @param index
     */
    public void trickleUp(int index){
        int parent =(index-1)/2; //父结点的索引
        Node bottom = heapArray[index]; //将新加入的尾节点存在bottom中;
        /*
           两个条件
           1.index>0代表存在父结点
    ...
    public Node remove() {
        ...
        //移除了最大值，需要找到一个原来第二大的值放在根节点
        trickDowm(0);
        return root;
    }
    /**
     * 向下调整(删除时需要向下调整，把删除原最大结点后移动到子节点的结点的合适的位置)
    ...
    public void trickDowm(int index){
        ...
        while (index<currentSize/2){
            //结点至少有一个子节点
            int leftChildIndex =2*index+1; //左子节点
            int rightChildIndex  =leftChildIndex+1; //右子节点
            if(rightChildIndex < currentSize && //右子节点等于空
